...
type StateDB struct {
	...
	// substrate-interpreter: EVM side-effect traces
	ResearchEVMTrace *research.EVMTrace
	// substate-interpreter: Current EVM CALL trace
	ResearchEVMCallTrace *research.EVMCallTrace
}
...
func New(root common.Hash, db Database, snaps *snapshot.Tree) (*StateDB, error) {
	...
	// substrate-interpreter: init StateDB.ResearchEVMTrace
	sdb.ResearchEVMTrace = research.NewEVMTrace()
	sdb.ResearchEVMCallTrace = nil
...
func (s *StateDB) AddLog(log *types.Log) {
	// substrate-interpreter: add log in EVM CALL trace
	s.ResearchEVMCallTrace.AddLogInstRet(log)
...
func (s *StateDB) Copy() *StateDB {
	...
	// substrate-interpreter: copy StateDB.ResearchEVMTrace
	state.ResearchEVMTrace = s.ResearchEVMTrace.Copy()
	state.ResearchEVMCallTrace = nil
	if s.ResearchEVMCallTrace != nil {
		for i, ct := range s.ResearchEVMTrace.CallTraces {
			if s.ResearchEVMCallTrace == ct {
				state.ResearchEVMCallTrace = state.ResearchEVMTrace.CallTraces[i]
				break
			}
		}
	}
