...
func run(evm *EVM, contract *Contract, input []byte, readOnly bool) ([]byte, error) {
	mode := evm.Config.InterpreterMode
	if research.GetContractIso() {
		if evm.Config.IsolationIndex == len(evm.StateDB.(*state.StateDB).ResearchEVMTrace.CallTraces)-1 {
			mode = evm.Config.IsolationMode
		} else {
			mode = InterpreterMode_EVM
		}

		switch mode {
		case InterpreterMode_EVM:
			evm.interpreter = evm.prevInterpreter
		case InterpreterMode_Substrate:
			evm.interpreter = NewSubstrateInterpreter(evm, evm.Config)
		case InterpreterMode_TAC:
			evm.interpreter = NewTACInterpreter(evm, evm.Config)
		default:
			panic(errors.Errorf("Unknown Interpreter mode: %v", mode))
		}
	}
	switch mode {
	case InterpreterMode_EVM:
		// Nothing to do for EVM
	case InterpreterMode_Substrate:
		// Try loading and parsing gigahorse substrate
		var exist bool
		if exist, contract.AstRoot = getAst(contract); !exist {
			return nil, nil
		}
		//Fuzz is not in use currently
		//research.PutFuncInfo(contract.Address(), contract.AstRoot.GetFuncPos())
	case InterpreterMode_TAC:
		var err error
		defer func() {
			// Delta.SubstrateTrace.CallTraces[0] is incomplete here,
			...
			if IsTACGlobalError(err) {
				evm.Delta.SubstrateErr = err
			}
		}()
		if contract.TACProg, err = getTACProg(contract); err != nil {
			return nil, err
		}
	default:
		panic(errors.Errorf("Unknown Interpreter mode: %v", mode))
...
type EVM struct {
	...
	// substrate-interpreter: EVM and interpreter delta semantics
...
func NewEVM(blockCtx BlockContext, txCtx TxContext, statedb StateDB, chainConfig *params.ChainConfig, config Config) *EVM {
	...
	checkEMI = research.GetDebugFlag()
	checkEMI = research.GetDebugFlag()

	switch mode := evm.Config.InterpreterMode; mode {
	case InterpreterMode_EVM:
		evm.interpreter = NewEVMInterpreter(evm)
		evm.prevInterpreter = evm.interpreter
	case InterpreterMode_Substrate:
		evm.interpreter = NewSubstrateInterpreter(evm, evm.Config)
	case InterpreterMode_TAC:
		evm.interpreter = NewTACInterpreter(evm, evm.Config)
...
func (evm *EVM) Call(caller ContractRef, addr common.Address, input []byte, gas uint64, value *uint256.Int) (ret []byte, leftOverGas uint64, err error) {
	...
	// substrate-interpreter: create new EVM CALL trace and restore old one later
	sdb := evm.StateDB.(*state.StateDB)
	// restore previous CALL trace when return
	defer func(prevCt *research.EVMCallTrace) {
		sdb.ResearchEVMCallTrace = prevCt
	...
	defer func(currCt *research.EVMCallTrace) {
		...
		currCt.CodeHash = evm.StateDB.GetCodeHash(addr)
		currCt.CallFn = "Call"
		currCt.Depth = int32(evm.depth + 1)
		currCt.Ret = ret
		currCt.LeftOverGas = leftOverGas
		currCt.RefundGas = evm.StateDB.GetRefund()
		currCt.Err = err
		currCt.Reverted = (err == ErrExecutionReverted)
	}(sdb.ResearchEVMCallTrace)
...
func (evm *EVM) Create(caller ContractRef, code []byte, gas uint64, value *uint256.Int) (ret []byte, contractAddr common.Address, leftOverGas uint64, err error) {
	...
	defer func(currCt *research.EVMCallTrace) {
		...
		currCt.CodeHash = research.CodeHash(code)
		currCt.CallFn = "Create"
		currCt.Depth = int32(evm.depth + 1)
		currCt.Ret = ret
		currCt.LeftOverGas = leftOverGas
		currCt.RefundGas = evm.StateDB.GetRefund()
