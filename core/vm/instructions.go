...
func opSload(pc *uint64, interpreter *EVMInterpreter, scope *ScopeContext) ([]byte, error) {
	...
	if checkEMI && checkLoad {
		t := fmt.Sprintf("address %s, loc: %s, value: %s, depth: %d, pc: %v", scope.Contract.Address().Hex(), hash.Hex(), val.Hex(), interpreter.evm.depth, *pc)
		fmt.Println("EVM SLOAD:", t)
	}
	return nil, nil
}
...
func opGas(pc *uint64, interpreter *EVMInterpreter, scope *ScopeContext) ([]byte, error) {
	// substrate-interpreter: push contract.Gas to EVM CALL trace
	interpreter.evm.StateDB.(*state.StateDB).ResearchEVMCallTrace.AddGasInstRet(scope.Contract.Gas)
...
func opCall(pc *uint64, interpreter *EVMInterpreter, scope *ScopeContext) ([]byte, error) {
	...
	if interpreter.evm.Delta != nil {
		return ret, interpreter.evm.Delta.SubstrateErr
	}
	return ret, nil
}
...
func makeLog(size int) executionFunc {
	return func(pc *uint64, interpreter *EVMInterpreter, scope *ScopeContext) ([]byte, error) {
		...
		// substrate-interpreter: add event log into trace
		interpreter.evm.StateDB.(*state.StateDB).ResearchEVMTrace.AddEventTrace(topics, d)
