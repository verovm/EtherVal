...
func (arguments Arguments) UnpackPosition(data []byte, pos int) (int, int, error) {
	nonIndexedArgs := arguments.NonIndexed()
	virtualArgs := 0
	for index, arg := range nonIndexedArgs {
		if arg.Type.T == ArrayTy && !isDynamicType(arg.Type) {
		...
		if index == pos {
			return getPosition((index+virtualArgs)*32, arg.Type, data)
		}
	}
	return -1, -1, nil
}

func getPosition(index int, t Type, output []byte) (int, int, error) {
	if index+32 > len(output) {
		return -1, -1, fmt.Errorf("abi: cannot marshal in to go type: length insufficient %d require %d", len(output), index+32)
	}
