...
func dbCompact(ctx *cli.Context) error {
	...
	go func() {
		...
		fmt.Printf("substate-cli db-compact: compacting EVM traces...\n")
		for b, m, s := 0, 0xffffffff, 0xffff; b <= m; b += s {
			start := research.Stage2EVMTraceKey(uint64(b), 0)
			end := research.Stage2EVMTraceKey(uint64(b+s), 0)
			if b+s >= m {
				end = research.Stage2EVMTraceKey(math.MaxUint64, math.MaxInt)
			}
			err = db.CompactRange(leveldb_util.Range{Start: start, Limit: end})
			if err != nil {
				panic(fmt.Errorf("substate-cli db-compact: error compacting dbPath %s: %w", dbPath, err))
		...
		fmt.Printf("substate-cli db-compact: compacting substrates...\n")
		for b := 0; b <= 255; b++ {
			start := research.Stage2SubstrateKey(common.Hash{byte(b)})
			end := research.Stage2SubstrateKey(common.Hash{byte(b + 1)})
			if b == 255 {
				end = research.Stage2SubstrateKey(common.MaxHash)
			}
			err = db.CompactRange(leveldb_util.Range{Start: start, Limit: end})
			if err != nil {
				panic(fmt.Errorf("substate-cli db-compact: error compacting dbPath %s: %w", dbPath, err))
