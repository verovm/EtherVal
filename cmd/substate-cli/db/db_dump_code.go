...
var DbDumpCodeCommand = &cli.Command{
	...
	Flags: []cli.Flag{
		...
		&cli.BoolFlag{
			Name:  "md5-name",
			Usage: "use MD5 for output file names if true, Keccak256 if false",
			Value: true,
		},
	},
	Description: `
	...
	for iterCount = 0; iter.Next(); iterCount++ {
		...
		if deployedOnly {
			...
			for _, codeHash := range codeHashSlice {
				...
				if md5Name {
					outputPath = filepath.Join(outputDir, research.Md5HashHex(code)+".hex")
				}
